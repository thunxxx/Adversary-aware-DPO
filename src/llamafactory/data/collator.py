...
@dataclass
class ATDataCollator(MultiModalDataCollatorForSeq2Seq):
    def __call__(self, features):
        utility_features = []
        chosen_features = []
        rejected_features = []
        for feature in features:
            if feature["chosen_input_ids"] is not None:
                chosen_feature = {
                    "input_ids": feature[f"chosen_input_ids"],
                    "attention_mask": feature[f"chosen_attention_mask"],
                    "labels": feature[f"chosen_labels"],
                    "images": feature["images"],
                    "videos": feature["videos"],                    
                }
                rejected_feature = {
                    "input_ids": feature[f"rejected_input_ids"],
                    "attention_mask": feature[f"rejected_attention_mask"],
                    "labels": feature[f"rejected_labels"],
                    "images": feature["images"],
                    "videos": feature["videos"],                      
                }
                ...
                utility_features.append(utility_feature)
        if len(utility_features) > 0:
            utility_features = super().__call__(utility_features)
        else:
            utility_features = None
        if len(chosen_features) > 0:
            chosen_features = super().__call__(chosen_features)
        else:
            chosen_features = None
        if len(rejected_features) > 0:
            rejected_features = super().__call__(rejected_features)
        else:
            rejected_features = None
        return(
            {
