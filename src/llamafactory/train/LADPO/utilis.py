...
class GDAdversary(torch.nn.Module):
    ...
    def forward(self, x):
        if x.shape[1] == 1 and self.attack.shape[1] != 1:  # generation mode (perturbation already applied)
            ...
            if x.shape[0] != self.attack_mask.shape[0]:        
                self.add = torch.cat((self.attack, self.attack), dim=0)
                self.mask = torch.cat((self.attack_mask, self.attack_mask), dim=0)


                perturbed_acts = x[self.mask[:, :x.shape[1]]] + self.add[self.mask[:, :x.shape[1]]].to(x.dtype)
                x[self.mask[:, :x.shape[1]]] = perturbed_acts
