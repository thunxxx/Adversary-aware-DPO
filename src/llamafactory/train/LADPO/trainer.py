...
class LADPOTrainer(DPOTrainer):
    ...
    def policy_forward(
        ...
        input_mask = (rejected_input_ids==rejected_labels).cumsum(dim=0) == 0
        pgd_layers=[8, 16, 24, 30]
        adversaries, hooks = add_hooks(model, pgd_layers, input_mask)
        rejected_features = {}
        rejected_features['input_ids'] = rejected_input_ids
        rejected_features['attention_mask'] = rejected_attention_mask
        rejected_features['labels'] = rejected_labels
        rejected_features['pixel_values'] = rejected_pixel_values     
        rejected_features['image_sizes']  = rejected_image_sizes 
