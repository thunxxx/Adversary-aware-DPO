...
class ADPOTrainer(DPOTrainer):
    ...
    def policy_forward(
        ...
        chosen_input_ids, rejected_input_ids = input_ids.split(batch_size, dim=0)
        chosen_attention_mask, rejected_attention_mask = attention_mask.split(batch_size, dim=0)
        chosen_labels, rejected_labels = labels.split(batch_size, dim=0)
        chosen_pixel_values, rejected_pixel_values = pixel_values.split(batch_size, dim=0)

        if self.model_name == 'llava_next':
            image_sizes = batch['image_sizes']
            chosen_image_sizes, rejected_image_sizes = image_sizes.split(batch_size, dim=0)
            adv_pixel_values = PGD(model, rejected_input_ids, rejected_attention_mask, rejected_pixel_values, rejected_labels, rejected_image_sizes)  
        else:
            adv_pixel_values = PGD(model, rejected_input_ids, rejected_attention_mask, rejected_pixel_values, rejected_labels)

        final_pixel_values = torch.cat((adv_pixel_values, adv_pixel_values), dim=0)

        batch['pixel_values'] = final_pixel_values
    ...
    def get_batch_loss_metrics(
        ...
        ) = self.policy_forward(model, batch)
